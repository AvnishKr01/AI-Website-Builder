...
// Controller Function To Make Rivision
export const makeRivision = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!message || message.trim() === '') {
            return res.status(400).json({ message: 'Please enter the valid prompt' })
        }
        ...
        if (!code) {
            await prisma.conversation.create({
                data: {
                    role: 'assistant',
                    content: "Unable to generate the code, Please try angain",
                    projectId
                }
            })
            await prisma.user.update({
...
// Controller For getting project code fot preview
...
// Controller for published Project
...
export const saveProjectCode = async (req: Request, res: Response) => {
    try {
        ...
        res.json({ message: "Project is saved Successfully" })
